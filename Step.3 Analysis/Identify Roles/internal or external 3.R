...
link <-read.csv(file = "/Users/mayhe/Desktop/thesis preparation/thesis drafts/thesis/data process/original analysis/Montreal/all_years/mobility&cooperation/mobi_link.csv",header = T,as.is = T)
node <-read.csv(file = "/Users/mayhe/Desktop/thesis preparation/thesis drafts/thesis/data process/original analysis/Montreal/all_years/mobility&cooperation/mobi_node.csv",header = T,as.is = T)
...
# > the node number since there are some including both internal and external
...
for (i in 2:obs2){
  if(coop[i,"Label"]==coop[i-1,"Label"]){
    coop[i,"if_cross"]<-NA
  }
}

coop2 <- coop[complete.cases(coop$if_cross),] 
# coop2 <- coop2[,-(2:3)]
